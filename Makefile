default:
	...
	# Linux 386
	CGO_ENABLED=0 GOOS=linux GOARCH=386 go build -o dist/linux_386/chalog main.go
	cp LICENSE dist/linux_386/LICENSE

	# Linux amd64
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o dist/linux_amd64/chalog main.go
	cp LICENSE dist/linux_amd64/LICENSE

	# Linux arm
	CGO_ENABLED=0 GOOS=linux GOARCH=arm go build -o dist/linux_arm/chalog main.go
	cp LICENSE dist/linux_arm/LICENSE

	# Linux arm64
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o dist/linux_arm64/chalog main.go
	cp LICENSE dist/linux_arm64/LICENSE

	# Darwin amd64
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o dist/darwin_amd64/chalog main.go
	cp LICENSE dist/darwin_amd64/LICENSE

	# Darwin arm64
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o dist/darwin_arm64/chalog main.go
	cp LICENSE dist/darwin_arm64/LICENSE

	# Windows 386
	CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -o dist/windows_386/chalog.exe main.go
	cp LICENSE dist/windows_386/LICENSE

	# Windows amd64
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o dist/windows_amd64/chalog.exe main.go
